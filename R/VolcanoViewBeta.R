...
#'
#' @docType methods
#' @name VolcanoViewBeta
#' @rdname VolcanoViewBeta
#'
...
VolcanoViewBeta <- function(df, x = "Betascore", y = "FDR", Label = NA, top = 5, topnames = NULL,
                        x_cutoff = 0.4, y_cutoff = 0.05, mycolour = c("gray80", "#e41a1c", "#377eb8"),
                        alpha = 0.6, force = 0.1, main = NULL, xlab = "B-Score", ylab = "-log10(FDR)",
  ...
  gg <- df
  names(gg) <- c( "Gene", "Betascore", "FDR")
  gg <- remove_rownames(gg)
  gg <- gg %>% column_to_rownames("Gene")

  # Took this formula from EnhancedVolcanoPlot
  if (min(gg[["FDR"]], na.rm = TRUE) == 0) {
    # <= v1.2
    ...
    warning(paste(
      "One or more FDR-values is 0.",
      "Converting to 10^-1 * current",
      "lowest non-zero p-value..."
    ),
    call. = FALSE
    )
    gg[which(gg[["FDR"]] == 0), "FDR"] <- min(
      gg[which(gg[["FDR"]] != 0), "FDR"],
      na.rm = TRUE
    ) * 10^-1
  }
  gg$group <- "no"
  ...
  p <- p + geom_point(aes_string(fill = "group"),
